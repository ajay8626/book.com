...
if(isset($_REQUEST['bu_name']) && $_REQUEST['bu_name'] != '' && isset($_REQUEST['bu_gender']) && $_REQUEST['bu_gender'] != '')
{
	...
	$_SESSION['sess_kids_dob'] = $datenum.'/'.$monthnumber.'/'.$yearnum; 
	//$_SESSION['sess_kids_dob'] = isset($_REQUEST['kids_dob'])?$_REQUEST['kids_dob']:"";	
			...
			<div class="mid" id="createbook">
				...
				<form name="homepageform" class="homepageform" action="" method="get">
					...
					<div class="nw_fld">
						...
						<div class="inputbox">
							<input type="text" placeholder="Enter Your Date of Birth" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Your Date of Birth'" class="" id="datepicker" name="kids_dob" maxlength="40" /><div id=""></div>
						</div>
						 */ ?>
                        
						<div class="dob">Enter Date of Birth</div>
						<select name="daynum">
							<option> Date </option>
							    <?php
							    	$rangeDate = range(1,31);
							    	foreach($rangeDate  as $day_of_month ) : ?>
							        <option value='<?php echo $day_of_month; ?>'><?php echo str_pad($day_of_month, 2, '0', STR_PAD_LEFT); ?></option>
							    <?php endforeach; ?>
					    </select>
					    <select name="monthnumber">
					    	<option>Month </option>
							    ...
							    foreach($rangeMonth  as $month_of_year ) : ?>
							        <option  value='<?php echo $month_of_year; ?>'><?php echo str_pad($month_of_year, 2, '0', STR_PAD_LEFT); ?></option>
							    <?php endforeach; ?>
					    </select>
					    <select name="yearnum">
				    		<option>Year</option>
							    <?php 
							    $rangeYear = range(2007,2019);
							    foreach( $rangeYear as $year ) : ?>
							        <option  value='<?php echo $year; ?>'><?php echo $year; ?></option>
							    <?php endforeach; ?>
				    	</select>
						<div class="" style="margin: 10px 0px 10px 0;"><i>(Book link will be emailed on this id.)</i></div>
