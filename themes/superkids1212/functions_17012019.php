...
function my_custom_checkout_field_display_admin_order_meta($order){
	echo '<p><strong>'.__('Street Address').':</strong>' . get_post_meta( $order->id, 'billing_house_number', true ) . ' ' .get_post_meta( $order->id, 'billing_street_address', true ) . '</p>';
	echo '<p><strong>'.__('Landmark').':</strong> <br/>' . get_post_meta( $order->id, 'billing_landmark', true ) . '</p>';
    echo get_post_meta( $order->id, 'billing_landline', true )?'<p><strong>'.__('Secondary Mobile Number').':</strong> <br/>' . get_post_meta( $order->id, 'billing_landline', true ) . '</p>':'';
	echo get_post_meta( $order->id, 'billing_secondry_email', true )?'<p><strong>'.__('Secondary Email Address').':</strong> <br/>' . get_post_meta( $order->id, 'billing_secondry_email', true ) . '</p>':''; 
    echo '<p><strong>'.__('Kid\'s Name').':</strong> <br/>' . get_post_meta( $order->id, 'billing_kidsname', true ) . '</p>';
}
...
function my_custom_kidsphoto_display_admin_order_meta($order){
	$kidsphoto = get_post_meta( $order->id, '_kidsphoto', true );
	if(!empty($kidsphoto)){
...
function my_custom_personalised_detail_display_admin_order_meta($order){
	/* $textval = get_post_meta( $order->id, '_personalised_detail', true );
    echo '<p><strong>'.__('Personalised details (First page text)').':</strong> <br></br> ' . nl2br($textval) . '</p>'; */
	
	$special_msg = get_post_meta( $order->id, '_special_message', true );
...
function wdm_add_values_to_order_item_meta_name_meaning($order){
	$name_meaning = get_post_meta( $order->id, 'name_meaning', true );
                    ...
                    <div class="personalised_detail_wrap">					
                        ...
                        <h3>Kid's Other Details</h3>
                            ...
                            <fieldset>
                                ...
                                <p class="form-row personalised_detail-class form-row-wide woocommerce-validated">
                                    <textarea name="special_message" style="width:100%; height:200px" maxlength="200"></textarea>
                                </p>
                            </fieldset>
...
function my_sms_cron_email() {
    if (! wp_next_scheduled ( 'checkout_hourly_event' )) {
		wp_schedule_event(time(), 'hourly', 'checkout_hourly_event');
...
function send_hourly_checkout_sms() {
	...
	$callingResult = $wpdb->get_results("SELECT * FROM `$callingTeamEmail` WHERE `visiting_page` = 'checkout'");
	$dailySmsCron = $wpdb->prefix.'daily_sms_cron';
	$cronSchedule = $wpdb->get_results("SELECT * FROM `$dailySmsCron` WHERE `ucode` = '$bcode' AND `visiting_page` = 'checkout'");
	$callingResult = $wpdb->get_results("SELECT * FROM `$callingTeamEmail` WHERE `visiting_page` = 'checkout'");
	...
	foreach($cronSchedule as $cronScheduleKey => $cronScheduleVal){
		$message = "Name : " . $_SESSION['sess_bookusername']. "<br> Unique Code : " . $cronScheduleVal->ucode;
		$subject = 'Checkout : '.$_SESSION['sess_bookusername'].' with code - '.$cronScheduleVal->ucode.' has visited checkout page in the site.';
...
function my_product_sms_cron_email() {
    if (! wp_next_scheduled ( 'product_hourly_event' )) {
		wp_schedule_event(time(), 'hourly', 'product_hourly_event');
    }
}
...
function send_hourly_product_sms() {
	...
	$bcode = $_SESSION['sess_bcode'];
	$dailySmsCron = $wpdb->prefix.'daily_sms_cron';
	$cronSchedules = $wpdb->get_results("SELECT * FROM `$dailySmsCron` WHERE `ucode` = '$bcode' AND `visiting_page` = 'product'");	
	$callingResult = $wpdb->get_results("SELECT * FROM `$callingTeamEmail` WHERE `visiting_page` = 'product'");
	if(!empty($callingResult)){
		foreach ($callingResult as $key => $value) {
			$getEmail[] = $value->email;		
		}
	...
	foreach($cronSchedules as $cronScheduleKeys => $cronScheduleVals){
		...
		$message = "Name : " . $_SESSION['sess_bookusername']. "<br> Unique Code : " . $cronScheduleVals->ucode;
		$subject = 'Product : '.$_SESSION['sess_bookusername'].' with code - '.$cronScheduleVals->ucode.' has visited Product page in the site';
		$Host = get_option('smtp_hostname');
		$Username = get_option('smtp_username');
		$Password = get_option('smtp_password'); 
		if($Host != '' && $Username != '' && $Password != ''){
