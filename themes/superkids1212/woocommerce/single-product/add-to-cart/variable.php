								...
								if ( taxonomy_exists( $name ) ) {
									...
									foreach ( $terms as $term ) {
										if($name == 'pa_height-chart'){
											$post_id = $wpdb->get_results("SELECT `ID` FROM `wp_posts` WHERE `post_title` = 'Superkids Height Chart (Measure upto 5 ft)' AND `post_parent` = '".$product->get_id()."' AND `post_status` = 'publish' ");
