...
function wc_cart_totals_coupon_label( $coupon, $echo = true ) {
	...
	if ( $echo ) {
		echo $label  . ' <a href="' . esc_url( add_query_arg( 'remove_coupon', urlencode( $coupon->get_code() ), defined( 'WOOCOMMERCE_CHECKOUT' ) ? wc_get_checkout_url() : wc_get_cart_url() ) ) . '" class="woocommerce-remove-coupon" data-coupon="' . esc_attr( $coupon->get_code() ) . '">' . __( '[Remove]', 'woocommerce' ) . '</a>';
	} else {
		return $label;
	}
}
...
function wc_cart_totals_coupon_html( $coupon ) {
	...
	$coupon_html          = $discount_amount_html . ' Discount';
