			...
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $term ) {
					if ( in_array( $term->slug, $options ) ) {
						if($term->slug == 'with-name'){
							$html .= '<input type="checkbox" name="variations" value="' . esc_attr( $term->slug ) . '" ' . selected( sanitize_title( $args['selected'] ), $term->slug, false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name ) ) ;
						}
						
						if(($term->slug == 'boy') ){
							$html .= '<input type="radio" 	name="gender" value="' . esc_attr( $term->slug ) . '" ' . selected( sanitize_title( $args['selected'] ), $term->slug, false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name ) ) ;
						}
					}
				}
			} else {
				foreach ( $options as $option ) {
					...
					$html .= '<input type="checkbox" name="variations" value="' . esc_attr( $option ) . '" ' . $selected . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $option ) ) ;
				}
			}
		}

		$html .= '</div>';
		echo apply_filters( 'woocommerce_checkbox_variation_attribute_options_html', $html, $args );
	}
}
