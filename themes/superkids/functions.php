...
function tl_continue_shopping_button() {
 $shop_page_url = site_url().'/kids-worksheet/';
    ...
    if ( $order->status != 'on-hold' && $order->status != 'pending' && $order->status != 'failed' && $order->status != 'neworder'){
		...
		if($phone != ''){
			...
			$log  = "User: ".$_SERVER['REMOTE_ADDR'].' - '.date("F j, Y, g:i a").PHP_EOL.
				...
				"phone:".$phone.PHP_EOL.
				"-------------------------".PHP_EOL;
			file_put_contents('./smslog/smslog_'.date("j.n.Y").'.txt', $log, FILE_APPEND);
		}
...
function my_custom_personalised_detail( $orderid ) {
	...
	$order = wc_get_order($orderid);
 	...
 	if($orderItemName == "Superkids Height Chart"){
 		if(empty($_kidsphoto)){
...
function Theme_css_js() {
	...
	wp_enqueue_style( 'croppie_css', get_template_directory_uri() . '/css/croppie.css', '', '3.7');
	wp_enqueue_style( 'bootstrap-3', get_template_directory_uri() . '/css/bootstrap-3.min.css', '', '3.7');
	...
	wp_enqueue_script( 'bootstrap-min', get_template_directory_uri() . '/js/bootstrap.min.js',array(),array(),false);
	wp_enqueue_script( 'jssor', get_template_directory_uri() . '/js/jssor.slider.min.js',array(),array(),false);
	wp_enqueue_script( 'croppie', get_template_directory_uri() . '/js/croppie.js',array(),array(),false);
...
function add_box_option_to_checkout( $checkout ) {
	...
	if(($prev_url != site_url().'/superhero-cape/') && ($prev_url != site_url().'/height-chart/') && ($bagpag[4] != 'bags-product')){
	...
	if($prev_url == site_url().'/height-chart/'){
		echo '<p><input type="radio" name="shipping_method" id="height_option" value="free_shipping" class="standard_delivery" checked="checked">
			<label for="height_option">Standard Delivery - 5 to 7 working days - <span style="font-weight:bold">Free</span></label>
			<br>
			<input type="radio" name="shipping_method" id="urgent_height_shipping" value="urgent_shipping" class="urgent_delivery">
...
function send_hourly_checkout_sms() {
	...
	//include_once(ABSPATH . 'class.phpmailer.php');
...
function send_hourly_product_sms() {
	...
	if(!empty($cronSchedules)){
		foreach($cronSchedules as $cronScheduleKeys => $cronScheduleVals){
			...
			$upload_dir = $upload['basedir'] . "/cron";
			$fileName = $upload_dir ."/cronfile.txt";
			$txt = 'Product Page Cron Run With Unique Code Unique Code in every five min. :  '.$cronScheduleVals->ucode.'----'.date('d/m/Y H:i').'---'.$getEmail[$i];
			$filedir = (file_exists($fileName)) ? fopen($fileName, "a") : fopen($fileName, "w");
			fwrite($filedir, $txt . "\n\n");
			fclose($filedir);
			...
			//update_option('ch_last_email_sent', $last_email_sent);
...
function load_kids_image(){
	...
	$kidsname = isset($_POST["kidsname"])?stripslashes($_POST["kidsname"]):"";
	$image_name = isset($_FILES["file"]["name"])?stripslashes($_FILES["file"]["name"]):"";
	...
	if($image_cont == "png") {
		...
		$path = imagejpeg($image, $upload_path."/kids-photo/".$png_name.".jpg",100);
		imagedestroy($image);
		if($path == 1) {
			$path = $upload_path."/kids-photo/".$png_name.".jpg";
		} else {
			$path = '';
		} 
	} else {
		$path = $upload_path."/kids-photo/".$image_name;
	...
	$kidsimage = $upload_path."/kids-photo/".$image_name;

	$pageImage = WP_PLUGIN_DIR.'/wp-book-virtue/images/heightchartimage.jpg';
	list($width,$height) = getimagesize($kidsimage);

	/*$image1 = imagecreatefromstring(file_get_contents($pageImage));
	$image2 = imagecreatefromstring(file_get_contents($kidsimage));*/

	$font_path = get_template_directory().'/fonts/P22Wedge.ttf';
	$text = $kidsname;
	$textlen = strlen($kidsname);

	$dest = imagecreatefromjpeg($pageImage);
	...
	imagedestroy($mask);
	imagettftext($dest, $size,$angle,$left,$top, $black, $font_path, $text);

	imagecopymerge($dest, $src, $dstX, $dstY, $srcX, 0, $src_width, $src_height, 100);

	header('Content-type: image/jpeg');
	//imagejpeg($dest);
	imagejpeg($dest, $upload_path."/kids-photo/new_image/".$image_name);
	imagedestroy($src);
	imagedestroy($dest);
	
	$newpath = $upload_url.'/kids-photo/new_image/'.$image_name;
	echo $newpath;
	
	/*imagecopymerge($image1, $image2, 850, 1200, 0, 0, $width, $height, 100);
	header('Content-Type: image/jpeg');
	imagejpeg($image1);*/
...
add_action('wp_enqueue_scripts', 'ajax_upload_height_image');
function ajax_upload_height_image(){
    wp_localize_script( 'at-jquery', 'ajax_upload_image', array(
                'ajax_url'   => admin_url( 'admin-ajax.php' ),
            )
    );
}
function upload_kids_image(){
	...
	imagejpeg($image1, $upload_path."/kids-photo/".$text."_".$rand_number.".jpg");
	$newpath = $upload_url.'/kids-photo/'.$text."_".$rand_number.".jpg";
	echo $newpath;
	die();

}
add_action( 'wp_ajax_upload_kids_image', 'upload_kids_image' );
add_action( 'wp_ajax_nopriv_upload_kids_image', 'upload_kids_image');
