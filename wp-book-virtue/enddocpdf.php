...
$dir = plugin_dir_path( __FILE__ );	
	if(!empty($certi_arr)) {
		...
		$point_y = 12.58;
		$margin = 1;
		$name = trim(ucfirst(strtolower($certi_name)));
		//$name = 'Pragnesh';
		$length = strlen($name);
		...
		$pdf->SetFont('KelmscottRegular','',22);
		$pdf->SetXY(22.26,12.50);
		$pdf->Cell(9.50,1.90,$name,0,1,'C');
		...
		foreach($certi_arr as $kname){
			$pdf->SetFont('KelmscottRegular','',20);
			$pdf->SetXY($point_x,$point_y);
			$pdf->Write(5,$kname[0]);
			$pdf->SetXY(24.52,$point_y);
			$pdf->Write(5," - ");
			$point_y = $point_y + $margin;
			$pdf->Write(5,trim(ucfirst(strtolower($kname[1]))));
		}
		$pdf->Output($order_id.'_'.trim(ucfirst(strtolower($certi_name))).'_ending.pdf',D);
	}
