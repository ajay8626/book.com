...
if(isset($_POST['submit'])){
  ...
  $kids_image = isset($_FILES["kids_image"]["name"])?stripslashes($_FILES["kids_image"]["name"]):"";
  $kids_temp = isset($_FILES["kids_image"]["tmp_name"])?($_FILES["kids_image"]["tmp_name"]):"";
  $path = plugin_dir_path(__FILE__)."preview_image/".$kids_image;
  move_uploaded_file($kids_temp, $path);
  $comment = isset($_POST['comments'])?$_POST['comments']:"";
	$gender = isset($_POST['gender'])?$_POST['gender']:"";

	$nameAttribute = isset($_POST['name_attribute'])?$_POST['name_attribute']:"";
	$data = array('kids_name' => $kname, 'gender' => $gender, 'name_attribute' => $nameAttribute,'kids_image' => $kids_image, 'comments' => $comment,'created_on' => date('Y-m-d H:i:s'));
...
<div class="container">
  <form name="preview_form" action="" method="post" enctype="multipart/form-data">
  <fieldset class="block">
    ...
    <?php /* <div class="form-group">
        <label for="text">Insert Image :</label>
        <input type="file" name="kids_image">
    </div>
    <div class="form-group">
